...
-- migration ledger (supabase_migrations.schema_migrations). The bundle is applied
-- with `psql -f`, which never writes the ledger, so it historically drifted far
-- behind the live schema (8 rows vs. 38 applied sections). Idempotent:
-- `on conflict (version) do nothing`, so re-running the bundle is safe.
-- ============================================================================
