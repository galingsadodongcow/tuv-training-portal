    ...
    <>
      ...
      {unstaffed.data?.length > 0 && (
        <div className="notice notice-info" style={{ marginBottom: 16 }}>
          ...
          {unstaffed.data.filter((u) => u.days_out <= 21).length > 0 && (
            <> {unstaffed.data.filter((u) => u.days_out <= 21).length} inside three weeks.</>
          )}{' '}
          <Link to="/calendar?month=all">Open the calendar</Link>
        </div>
      )}
