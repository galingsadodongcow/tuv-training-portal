...
export async function getCommercialWorkspace(): Promise<CommercialWorkspace> {
  ...
  const results = await Promise.all([
    ...
    supabase.from('quotation_lines').select('id, quotation_id, course_id, learning_type, participant_count, unit_price, currency'),
    supabase.from('orders').select('id, order_number, quotation_id, inquiry_id, customer_id, contact_id, sales_owner_id, operations_owner_id, status, requested_start_date, delivery_notes, operations_note, handoff_sent_at, reviewed_at, created_at').order('created_at', { ascending: false }),
    supabase.from('order_lines').select('id, order_id, course_id, learning_type, participant_count, unit_price, currency'),
