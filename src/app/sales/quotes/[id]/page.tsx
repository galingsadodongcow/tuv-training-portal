        ...
        <section className="workspace-section" aria-labelledby="quote-lines-title">
          ...
          {quotation.status === 'draft' ? <form action={addQuotationLineAction} className="workflow-form form-spaced">
            ...
            <div className="field-grid field-grid-three">
              ...
              <label className="field"><span>Delivery plan</span><select name="delivery_intent" defaultValue="operations_to_assign"><option value="operations_to_assign">Let Operations assign (recommended)</option><option value="existing_session">Reserve existing public session</option><option value="private_session">Create private session</option></select></label>
              <label className="field field-wide"><span>Public session <small>required only for existing-session delivery</small></span><select name="session_id" defaultValue=""><option value="">Choose a published public session</option>{workspace.sessions.filter((session) => session.offering_type === 'public' && session.publication_status === 'published' && ['scheduled', 'open'].includes(session.status)).map((session) => <option value={session.id} key={session.id}>{displayNumber('SES', session.session_number)} · {courseName.get(session.course_id)} · {session.learning_type} · {new Intl.DateTimeFormat('en-PH', { dateStyle: 'medium', timeZone: 'Asia/Manila' }).format(new Date(session.starts_at))}</option>)}</select></label>
            </div>
            <p className="form-help">Existing-session selections are validated against course and modality, then capacity is reserved atomically at order creation. Standard references: {workspace.prices.slice(0, 6).map((price) => `${courseName.get(price.course_id)} (${price.learning_type}: ${money(Number(price.amount), price.currency)})`).join(' · ')}</p>
