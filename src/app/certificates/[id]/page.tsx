...
export default async function CertificateDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell profile={profile} active="participants">
    ...
    <div className="page-heading"><div><p className="eyebrow">Controlled certificate document</p><h1>{participant.certificate_number}</h1><p>Issued from a completed, eligible training record.</p></div><a className="button" href={`/api/certificates/${participant.id}/pdf`} download>Download PDF</a></div>
