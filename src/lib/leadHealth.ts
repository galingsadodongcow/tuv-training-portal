...
// a {label, cls} using the existing pill vocabulary from globals.css, or null
// when there's nothing worth flagging so the UI stays quiet.

import { daysUntil } from './format'

export interface LeadBadge {
  label: string
  cls: string // an existing pill-* class in globals.css
}
...
export function inquiryHealth(inq: any): LeadBadge | null {
  ...
  if (status === 'Closed Won') return { label: 'Won', cls: 'pill-cancelled' }
  if (status === 'Closed Lost') return { label: 'Lost', cls: 'pill-cancelled' }
  ...
  if (age > AGE_DANGER) return { label: 'Stalled', cls: 'pill-nogo' }
  if (age > AGE_WARN) return { label: 'Ageing', cls: 'pill-tentative' }
  return null // fresh
}
...
export function quoteHealth(q: any): LeadBadge | null {
  ...
  if (status === 'Accepted' || q.converted_order_id) return { label: 'Accepted', cls: 'pill-cancelled' }
  ...
  if (left < 0) return { label: 'Expired', cls: 'pill-nogo' }
  if (left <= EXPIRY_WARN) return { label: 'Expiring', cls: 'pill-tentative' }
  return null
}
